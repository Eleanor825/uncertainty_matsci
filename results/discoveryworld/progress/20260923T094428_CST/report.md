# P335 completed: observational score differences with environmental divergence
...
**Mechanism qualification (subsequent audit):** the first13 executed actions, prompt tokens, seeds and token log-probabilities matched across all four audited pairings, but public environment state and prompts diverged at14 before a controller-induced packet change. Both conditional arms always selected their first candidate. The+0.125 and+0.25 score contrasts below are therefore **observational; environment-prefix mismatch prevents isolating a causal feedback/controller benefit**. The exact environment root cause is unknown, and no Python-hash or LLM bug is established. [Detailed mechanism note](mechanism_note.md).
