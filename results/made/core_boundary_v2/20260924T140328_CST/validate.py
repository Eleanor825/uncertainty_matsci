...
if s['benchmark']=='MADE':
 assert s['all_status_counts']=={'accepted':73,'active_or_claimed':2,'pending':45}
 assert [budgets[b]['complete_pairs']for b in(10,20,30)]==[20,16,0]
 assert Counter(r['state']for r in rows if r['origin']=='new_v2')==s['new_status_counts']=={'accepted':69,'active_or_claimed':2,'pending':45}
 assert s['fixed_reuse_accepted']==4 and s['previous31_complete_refs_and_metrics_identical']
 assert budgets[20]['group_counts']=={'Au-K-Tb':8,'Mg-Sn-Sr':8}
 for g in s['descriptive_statistics']:
 ...
 assert budgets[20]['metrics']['SUN']['win_tie_loss']=={'wins':4,'ties':4,'losses':8}
else:
