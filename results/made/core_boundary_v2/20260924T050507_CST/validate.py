...
assert Counter(r['state']for r in rows)==s['all_status_counts']=={'accepted':20,'active_or_claimed':2,'pending':98}
assert Counter(r['state']for r in rows if r['origin']=='new_v2')==s['new_status_counts']=={'accepted':16,'active_or_claimed':2,'pending':98}
assert sum(r['origin']=='fixed_reuse'and r['accepted']for r in rows)==4
complete=[p for p in pairs if p['accepted_pair']];assert len(complete)==s['complete_pairs']==9
for p in pairs:
...
assert {x['same_birth_live_job_id']for x in s['native_device_observations']}=={r['job_id']for r in rows if r['state']=='active_or_claimed'}
...
print(json.dumps({'status':'passed','accepted':20,'complete_pairs':9,'new_accepted':16,'reused':4,'active_or_claimed':2,'pending':98,'new_scientific_calls':0},sort_keys=True))
