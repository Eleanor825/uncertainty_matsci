...
def reconstruct_scientific_evidence(output: Path, job: Mapping, *, tasks: Mapping, partition: str = "training",
                                    ...
                                    core_protocol=None, made_all30_extension=None, made_budget_sweep=None) -> dict:
    ...
    for rpc_request, response, _ in pairs[1:]:
        ...
        if job["benchmark"] == "made":
            ...
            _require(type(last_stable) in (int, float) and curve[-1][1] <= last_stable <= attempt, "Invalid official discovery count")
