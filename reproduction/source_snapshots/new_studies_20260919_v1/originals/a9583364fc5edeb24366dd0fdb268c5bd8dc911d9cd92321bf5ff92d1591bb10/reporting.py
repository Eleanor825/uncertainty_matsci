"""Original seven-arm metrics/reporting, with explicit imported150/new2150 accounting."""
...
def build(reg,acceptance):
    root=Path(reg['output']);protocol=read(reg['original_protocol']['path']);data={};summaries={}
    ...
    result={"schema":"summit_snar_main_repair_complete_report_v2","imported_collection_oracle_calls":150,"new_adaptation_oracle_calls":400,"new_test_oracle_calls":1750,"protocol_fingerprint":digest(protocol),"adaptation_oracle_calls":550,
    ...
    with (root/"results.md").open("x") as stream:stream.write("\n".join(lines)+"\n")
    ...
    return [root/'results.json',root/'results.md',root/'study_complete.json']
