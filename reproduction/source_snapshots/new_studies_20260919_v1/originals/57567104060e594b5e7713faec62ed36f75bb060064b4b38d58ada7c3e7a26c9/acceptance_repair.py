...
def _oracle_evidence(root, protocol, expected, ev, source_lock, source_lock_sha):
    def physical_scope(spec):
        return spec['scope']+'__seed'+str(spec['seed']) if spec.get('role')=='test' else spec['scope']
    expected_scopes={physical_scope(x) for x in expected.values()}
    all_scopes={physical_scope(x) for x in expected_episodes(protocol).values()}
    actual={p.name:p for p in (root/'oracle_journals').iterdir() if p.is_dir()}
    require(expected_scopes <= set(actual) <= all_scopes,'Unexpected/missing repair oracle scope')
    folders={name:actual[name] for name in expected_scopes}
    scope_queries={name:{spec['name']+f'/q{i:03d}' for spec in expected.values()
                        if physical_scope(spec)==name for i in range(spec['budget'])} for name in folders}
    logical_scope={name:{spec['scope'] for spec in expected.values() if physical_scope(spec)==name}.pop() for name in folders}
    ...
    for scope,folder in sorted(folders.items()):
        ...
        for file in intents:
            ...
            require(file.name==key+'.intent.json' and qid not in by_query and qid in scope_queries[scope],'Duplicate/wrong query identity')
            ...
            by_query[qid]={'scope':logical_scope[scope],'physical_scope':scope,'result':result,'terminal_sha256':ev.files[str(file.with_name(key+'.terminal.json').resolve())]['sha256']}
        require(len(starts)==len(scope_queries[scope]) and identity['max_queries']==len(starts),'Incomplete or wrong-cap repair oracle scope')
        counts[logical_scope[scope]]=counts.get(logical_scope[scope],0)+len(starts)
    require(counts.get('fit_risk_default',0)==0,'Risk fitting called the oracle')
    return by_query,counts


def _evolution(root,protocol,episodes,ev,base,*,branches=("full","es_only"),require_clean_replay=True):
...
def accept_study(output,protocol):
    ...
    report={'schema':'summit_snar_main_repair_terminal_acceptance_v2','passed':True,'complete':True,
...
def audit_episodes(root,protocol,expected,physical,ev,prior,base_record):
    base=base_record['base_state_hash']
    folders={name:Path(root)/'episodes'/name for name in expected}
    summaries={};all_rows={};adapt_rows=[]
    for name,spec in sorted(expected.items()):
        folder=folders[name];header=ev.read(folder/'episode.json');p=prior if spec.get('role')=='test' else []
        require(all(header[k]==spec[k] for k in ('name','arm','seed','budget')) and header['protocol_fingerprint']==digest(protocol)
        ...
        summaries[name]=actual
        if spec.get('role')!='test':adapt_rows.extend(rows)
    return summaries,all_rows,adapt_rows


def audit_job(reg,job):
    """Read-only complete branch/test validation against real closed Oracle journals."""
    from common import read as read_json, check, verify_import
    verify_import(reg)
    root=Path(reg['output']);protocol=read_json(check(reg['original_protocol']));catalog=expected_episodes(protocol)
    if job['kind']=='evolve':expected={k:v for k,v in catalog.items() if v.get('branch')==job['branch']}
    elif job['kind']=='evaluate':expected={job['job_id']:catalog[job['job_id']]}
    else:raise AcceptanceError('Only exact registered branch or evaluation jobs are accepted')
    require(sum(x['budget'] for x in expected.values())==job['queries'],'Job budget scope differs')
    ev=Evidence();source_refs=[x for x in reg['sources'] if Path(x['path']).name=='source_lock.json']
    require(len(source_refs)==1,'Official source lock ambiguous')
    source_lock=ev.read(check(source_refs[0]));source_sha=source_refs[0]['sha256']
    physical,counts=_oracle_evidence(root,protocol,expected,ev,source_lock,source_sha)
    prior=ev.read(root/'shared_prior_archive.json') if job['kind']=='evaluate' else []
    base=ev.read(root/'base_policy.json')
    summaries,rows,_=audit_episodes(root,protocol,expected,physical,ev,prior,base)
    require(set(rows)==set(physical),'Extra/missing query evidence')
    selected=None
    if job['kind']=='evolve':
        selected=_evolution(root,protocol,summaries,ev,base['base_state_hash'],branches=(job['branch'],),require_clean_replay=False)
    for name,spec in expected.items():
        state=ev.read(root/'executed_policy_states'/f'{name}.json')
        require(state['episode']==name and state['actual_weight_hash']==summaries[name]['weight_hash']
                and state['computed_from_actual_parameter_tensors'] is (spec['arm'] not in ('random','gp_ei_scalarized')),'Missing actual episode policy state proof')
        if spec['arm'] in ('random','gp_ei_scalarized'):
            require(state['actual_weight_hash'] is None,'CPU baseline has a policy state')
        elif spec.get('role')=='population':
            stamp=state['model_stamp'];require(stamp['generation']==spec['generation']
                and stamp['perturbation_seed']==spec['mutation'] and stamp['perturbation_sigma']==protocol['evolution']['sigma'][spec['generation']],
                'Population used wrong actual perturbation')
        elif spec.get('role')=='dev':
            require(state['model_stamp']['generation']==spec['generation'],'Development used wrong generation')
        if spec.get('role')=='test':
            arm=spec['arm'];expected_hash=None if arm in ('random','gp_ei_scalarized') else base['base_state_hash']
            if arm in ('uq_esopt','es_only'):
                branch='full' if arm=='uq_esopt' else 'es_only'
                selection=ev.read(root/f'{branch}_checkpoint_selection.json');expected_hash=selection['selected']['weight_hash']
            require(state['actual_weight_hash']==expected_hash,'Final episode used wrong selected/original weights')
    report={'schema':'snar_repair_job_domain_validation_v2','passed':True,'job_id':job['job_id'],
            'queries':len(rows),'oracle_stage_counts':counts,'selection':selected,
            'new_scientific_calls_by_audit':0,'evidence_files':sorted(ev.files.values(),key=lambda x:x['path'])}
    report['fingerprint']=digest(report);return report
