"""Independent MADE support-aware full ES training, never a rename of the old full-method G2.
...
from matdiscovery.support_scope import admitted_parent, read_registration, validate_launch
from matdiscovery.rollouts import DiscoveryRollout, RolloutSettings
from arm_runtime import SupportAwareRollout
...
SCHEMA = 'made_independent_full_es_complete_v1'
JOB_SOURCE = 'registered_independent_MADE_support_aware_full_ES_B10_training_v1'
...
def publish(path,value):
    ...
    try:
        ...
        try:os.link(temporary,path)
            ...
            require(read(path)==value,'Refuse to replace immutable support-aware full ES evidence: '+str(path))
    finally:temporary.unlink(missing_ok=True)


def _source_bound(reg):
    require(artifact(__file__) in reg['external_sources'],'Executing support-aware full ES source is not registered')
    for item in reg.get('frozen_NN_artifacts',[]):require(artifact(item['path'])==item,'Frozen NN bytes changed')
...
class FullESCondition:
    ...
    def protocol(self):
        ...
        require(declared_mace_workers(protocol)==4,'support-aware full ES needs the same MACE4/ORB1 evaluator')
        return protocol

    def config(self):
        es=self.protocol()['esopt']
        return ESTrainingConfig(benchmark='made',method='esopt_graph_risk',seed=1,dev_every=1,
    ...
    def cases(self):
        ...
        for split,task in expected.items():
            require(self.core[split+'_tasks']==[task],'support-aware full ES may use only the original fixed train/dev chemistry')
...
def condition_for(reg):
    ...
    return FullESCondition(Path(core['workspace']).resolve(),core)


def condition_directory(reg): return Path(reg['workspace']).resolve()/'experiments/full_es'
...
def _builder(condition,request,entry,configuration_fingerprint,reg):
    ...
    job.update(job_id='support-full-es-'+request.request_id,budget=10,
               ...
               ablation_training_scope='support_aware_full_es',risk_used=True,
               uncertainty_reward_weight=0.0,controller_enabled=True)
    return job
...
def validate_training_job_scope(reg,job,expected_made_budget,core_protocol=None):
    ...
    require(type(expected_made_budget) is int and expected_made_budget==10,'Independent support-aware full ES budget is exactly B10')
    ...
    require(request.method=='esopt_graph_risk' and request.run_fingerprint==planner.run_fingerprint
...
def _support_rows(output,request):
    ...
    for row in rows:
        ...
        require(row['generation']['seed']==(request.environment_seed*10000019+sequence*97+candidate)%2**63,
                'Support-aware full ES generation RNG changed')
...
def load_controllers(condition,policy,reg):
    ...
    from matdiscovery.core_collection import corpus_contract
...
class FullESCallbacks(TrainingJobCallbacks):
    def __init__(self,condition,policy,directory,reg,*,settings,risk_model,attributor,auxiliary_files,rollout_factory=None):
        require(settings.failure_aware_control is True,'Support-aware risk control must be enabled')
        super().__init__(condition,policy,Path(directory)/'run',settings=settings,risk_model=risk_model,attributor=attributor,
            auxiliary_files=[*auxiliary_files,Path(__file__).resolve(),Path(reg['workspace'])/'configs/support_aware_method.json'],
            rollout_factory=rollout_factory or SupportAwareRollout)
        ...
        self.callback_identity.update(schema=SCHEMA,ablation_fingerprint=reg['fingerprint'],
            support_contract='native_graph_support_abstention_v1',risk_used=True,
            controller_enabled=True,uncertainty_reward_weight=0.0,made_episode_budget=10,
    ...
    def _scientific(self,request,job,output):
        value=reconstruct_scientific_evidence(output,job,tasks=self.tasks,partition='training',
            expected_mace_num_workers=4,expected_made_budget=10,support_registration=self.reg)
        _support_rows(output,request)
        return value
    ...
    def evaluate(self,policy,job,request):
        ...
        self.rollout_factory(self.condition.project,policy,settings=self.settings,risk_model=self.risk_model,attributor=self.attributor).run(job,output,collection=False)
    ...
    def result_verifier(self,request,result):
        ...
        require(evidence==receipt['evidence'] and fingerprint(asdict(result))==fingerprint(asdict(self._result(request,receipt))),
                'support-aware full ES reward or costs not official evidence')
        return {'verified':True,'evidence':'Independent six-slot training scope; closed official RPC/ORB+MACE journal and support-aware graph control',
...
def audit_full_es(condition_dir,*,registration):
    ...
    require(identity==planner.identity and manifest['run_fingerprint']==planner.run_fingerprint==summary['run_fingerprint'],
            'support-aware full ES config/RNG/source run identity changed')
    ...
    require(descriptor['callback_identity']['risk_used'] is True and descriptor['callback_identity']['controller_enabled'] is True
            and descriptor['callback_identity']['uncertainty_reward_weight']==0
            and descriptor['callback_identity']['support_contract']=='native_graph_support_abstention_v1','Wrong uncertainty/control contract')
    ...
    for path in files:
        ...
        raw=reconstruct_scientific_evidence(output,job,tasks=tasks,partition='training',expected_mace_num_workers=4,
                                            expected_made_budget=10,support_registration=registration)
        _support_rows(output,request)
    ...
    report={'verification_schema':'six_real_support_aware_full_ES_B10_RPC_trajectories_v1','complete':True,
        ...
        'candidate_oracle_attempts':60,'risk_used':True,'controller_enabled':True,'uncertainty_reward_weight':0,
        'support_contract':'native_graph_support_abstention_v1',
...
def verify_full_es_receipt(condition_dir=None,*,registration):
    ...
    report,summary,markers=audit_full_es(directory,registration=registration)
    ...
    require(receipt['evaluations']==6 and receipt['candidate_oracle_attempts']==60 and receipt['risk_used'] is True
            and receipt['controller_enabled'] is True and receipt['uncertainty_reward_weight']==0
            and receipt['support_contract']=='native_graph_support_abstention_v1'
    ...
    return {'support_aware_full_es':True,'training_method':'esopt_graph_risk','risk_used':True,'support_contract':'native_graph_support_abstention_v1','selected_generation':best,
...
def load_full_es_selected(policy,condition_dir=None,*,registration):
    condition=condition_for(registration)
    profile=verify_full_es_receipt(condition_dir,registration=registration)
    ...
    policy.mark_state('independent_full_es_selected_reload',generation=best)
    return profile


def _run_full_es_locked(workspace,*,resume=False,plan_only=False,policy_factory=None,rollout_factory=None,
                clean_reload_validator=cpu_clean_reload_validator,controller_loader=None):
    ...
    if plan_only:
        ...
        return {'planned_only':True,'condition':{'method':'esopt_graph_risk','risk_used':True},'config':asdict(config),
    ...
    settings=RolloutSettings(max_generation_retries=protocol['risk_network']['maximum_candidate_generations_per_decision'],
        ...
        activation_tokens_per_decision=protocol['collection']['activation_token_rows_per_complete_decision_prefix'],failure_aware_control=True)
    risk,attributor,auxiliary=(controller_loader or load_controllers)(condition,policy,reg)
    callbacks=FullESCallbacks(condition,policy,directory,reg,settings=settings,risk_model=risk,attributor=attributor,
        auxiliary_files=auxiliary,rollout_factory=rollout_factory)
    ...
    if (directory/'training_receipt.json').exists():
        ...
        return load_full_es_selected(policy,directory,registration=reg)
    ...
    report,summary,markers=audit_full_es(directory,registration=reg)
    best=report['selected_generation'];checkpoint=directory/'run/checkpoints'/f'generation_{best:04d}'/'policy.pt'
    optimizer=_load_selected(policy,checkpoint,report['selected_actual_model_state_hash'],config)
    policy.mark_state('independent_full_es_selection',generation=best)
    ...
    receipt={'schema':SCHEMA,'complete':True,'ablation_fingerprint':reg['fingerprint'],
        ...
        'clean_reload_passed':True,'evaluations':6,'candidate_oracle_attempts':60,'risk_used':True,
        'uncertainty_reward_weight':0,'controller_enabled':True,'test_used':False,
        'support_contract':'native_graph_support_abstention_v1',
...
def run_full_es(workspace,*,resume=False,plan_only=False,**kwargs):
    if plan_only:return _run_full_es_locked(workspace,resume=resume,plan_only=True,**kwargs)
    import fcntl,socket,time
    reg=read_registration(workspace);validate_launch(reg);directory=condition_directory(reg)
    directory.mkdir(parents=True,exist_ok=True)
    with (directory/'.execution.lock').open('a') as lock:
        fcntl.flock(lock,fcntl.LOCK_EX|fcntl.LOCK_NB)
        path=directory/'invocation.json'
        if path.exists():
            require(resume and read(path)['support_fingerprint']==reg['fingerprint'],
                    'Existing full ES invocation requires explicit no-replay reconciliation/resume')
        else:
            require(not resume,'Cannot resume unstarted full ES')
            with path.open('x') as f:
                json.dump({'support_fingerprint':reg['fingerprint'],'pid':os.getpid(),'hostname':socket.gethostname(),
                           'started_at':time.time(),'automatic_physical_replay':False},f)
                f.flush();os.fsync(f.fileno())
        return _run_full_es_locked(workspace,resume=resume,**kwargs)
...
def main():
    ...
    with immutable_hash_cache():
        if args.execute:
            ...
            with exclusive_resource(reg,args.gpu_uuid):result=run_full_es(args.workspace,resume=args.resume)
        else:result=run_full_es(args.workspace,resume=args.resume,plan_only=True)
