...
SCHEMA='MADE_support_aware_fixed_NN_method_v1'
SOURCE='registered_support_aware_evaluation'
...
ARMS={'uq_only_support_aware':'graph_risk','full_support_aware':'esopt_graph_risk'}
COMPONENT_FP='216fa4f7df3dfc25b66d4e00a2e4788c4f2a2b3c2f8d563eae231e892cdcbed6'
BUDGETS=(10,30,50)
...
def external_inventory():
    names=('registration.py','full_es_training.py','arm_runtime.py','evaluate.py','resource_guard.py','source_delta.py','shared_references.py','decision_validation.py')
        ...
        for seed in EVALUATION_SEEDS:
            for arm,route in ARMS.items():
                for task_id,task in sorted(tasks.items(),key=lambda pair:(len(pair[1]['elements']),pair[0])):
                    ...
                    job.update(study_id=SCHEMA,classification='post_seed1_training_dev_support_contract_repair',source=SOURCE,
                    ...
                    job['job_id']='support-final-'+fingerprint(job)[:24];jobs.append(job)
    require(len(jobs)==len({j['job_id'] for j in jobs})==540,'Wrong component matrix')
    return jobs


def expected_training_source(original):
    old='core_protocol=None, made_all30_extension=None, made_budget_sweep=None) -> dict:'
    new='core_protocol=None, made_all30_extension=None, made_budget_sweep=None, support_registration=None) -> dict:'
    branch='''    if support_registration is not None:
        ...
        from .support_scope import validate_support_reconstruction
        validate_support_reconstruction(support_registration, job, expected_made_budget, partition)
...
def verify_runtime_sources(value):
    ...
    require(len(original)==52 and {r['relative_path'] for r in value['runtime_sources']}==set(original)|{'support_scope.py','support_control.py'},'Scientific source inventory changed')
    for name,p in original.items():
        ...
        elif name=='rollouts.py':
            from source_delta import expected_rollout_source
            require((package/name).read_text()==expected_rollout_source(p.read_text()),'Unexpected support rollout source delta')
...
def component_reference(reference,parent):
    verify_artifact(reference);value=read(reference['path'])
    require(value['schema']=='MADE_fixed_artifact_component_ablations_v1' and value['fingerprint']==COMPONENT_FP
            and value['fingerprint']==fingerprint({k:v for k,v in value.items() if k!='fingerprint'}),'Wrong sealed component reference')
    require(value['parent']['sha256']==artifact(Path(parent['workspace'])/'configs/made_budget_sweep.json')['sha256']
            and value['baseline_reference_profile']==parent['execution_profiles']['baseline'],'Shared reference ancestry/profile differs')
    require(value['evaluation_seeds']==list(EVALUATION_SEEDS) and value['budgets']==list(BUDGETS)
            and len(value['new_jobs'])==810,'Shared reference scope differs')
    selected=[j for j in value['new_jobs'] if j['arm_id'] in ('baseline_reference','es_only_independent')]
    tasks={j['task_id'] for j in parent['new_jobs']}
    expected={(a,b,s,k) for a in ('baseline_reference','es_only_independent') for b in BUDGETS for s in EVALUATION_SEEDS for k in tasks}
    require(len(selected)==540 and {(j['arm_id'],j['budget'],j['seed'],j['task_id']) for j in selected}==expected,
            'Shared540 matrix is incomplete or duplicated')
    return deepcopy(selected)


NN_SHA={'graph_risk.pt':'315d990a76f1329c4299a8be04dc710e47d23eb1e09f4563acb963f5ce1a2173',
        'graph_risk.failure_heads.pt':'70c6fc0cb79bbf2f3548e1f4ea23fdfc3505d5b39e2b5bc88df8f7d5be870279'}

def frozen_risk_artifacts(parent):
    root=Path(parent['parent_core']['workspace'])/'experiments/risk_models/qwen35_4b/made'
    refs=[artifact(root/name) for name in NN_SHA]
    require(all(r['sha256']==NN_SHA[Path(r['path']).name] for r in refs),'The fitted original scalar/type NN changed')
    return refs


def build_registration(parent_reference,workspace,*,validation,component_registration):
    ...
    component=artifact(component_registration);shared=component_reference(component,parent)
    old_component=Path(read(component['path'])['workspace']).resolve()
    require(workspace!=old_component and old_component not in workspace.parents and workspace not in old_component.parents,'Cannot write into the original component workspace')
    value={'schema':SCHEMA,'workspace':str(workspace),'parent':reference,'parent_workspace':parent['workspace'],
        ...
        'new_jobs':planned_jobs(parent),'expected_counts':{'jobs':540,'episodes':540,'candidate_oracle_attempts':16200,'dft_episode_attempts':0},
        ...
        'validation':validation_ref,'component_reference':component,
        'shared_reference_jobs':shared, 'support_contract':'native_graph_support_abstention_v1',
        'NN_refit':False, 'legacy_UQ_only_included':False,'frozen_NN_artifacts':frozen_risk_artifacts(parent),
        'candidate_selection_data':'original_MADE_train_dev_only_not_test_outcomes',
        'future_evaluation_independent_of_training_seeds':True,
        'historical_test_seed1_already_observed':True,'baseline_full_new_seed_results_required_for_matched_comparison':True,
        'training_and_evaluation_seeds_are_distinct_roles':True,'reuse_original_full_G2_as_new_full':False,
        'UQ_only_weights':'exact_original_theta0','full_ES_initial_weights':'exact_original_theta0',
        'full_ES_reward':'official_AUDC_only','full_ES_support_aware_control':True,
    ...
    path=workspace/'configs/support_aware_method.json';require(not path.exists(),'Preserve original registration')
...
def validate_registration(value):
    ...
    require(value['arms']==ARMS and value['budgets']==list(BUDGETS) and value['evaluation_seeds']==list(EVALUATION_SEEDS) and value['new_jobs']==planned_jobs(parent),'Exact540 evaluation scope changed')
    ...
    require(value['expected_counts']=={'jobs':540,'episodes':540,'candidate_oracle_attempts':16200,'dft_episode_attempts':0},'Ablation costs changed')
    ...
    require(value['reuse_original_full_G2_as_new_full'] is False and value['full_ES_support_aware_control'] is True and value['full_ES_reward']=='official_AUDC_only','Independent ES semantics changed')
    fixed={'historical_test_seed1_already_observed':True,'baseline_full_new_seed_results_required_for_matched_comparison':True,
           'training_and_evaluation_seeds_are_distinct_roles':True,'UQ_only_weights':'exact_original_theta0',
           'full_ES_initial_weights':'exact_original_theta0','initial_ES_dev_evaluation':False,
    ...
    require(value['frozen_NN_artifacts']==frozen_risk_artifacts(parent),'Fitted NN binding changed')
    require(value['shared_reference_jobs']==component_reference(value['component_reference'],parent),'Shared baseline/ES-only reference matrix changed')
    require(value['support_contract']=='native_graph_support_abstention_v1' and value['NN_refit'] is False and value['legacy_UQ_only_included'] is False
            and value['candidate_selection_data']=='original_MADE_train_dev_only_not_test_outcomes'
            and value['future_evaluation_independent_of_training_seeds'] is True,'Support contract or old-UQ inclusion changed')
    return value


def read_registration(workspace):
    path=Path(workspace).resolve();path=path/'configs/support_aware_method.json' if path.is_dir() else path
    value=validate_registration(read(path))
    require(path==Path(value['workspace'])/'configs/support_aware_method.json','Registration workspace path differs')
    return value


def _deep_parent_validation(value):
    folder=Path(value['parent_workspace'])/'src/matdiscovery';name='_support_original_'+PARENT_FP[:12]
...
def validate_launch(value):
    ...
    return {'complete':True,'support_fingerprint':value['fingerprint'],'parent_proof':deepcopy(_ADMITTED[key]['proof'])}
...
def load_es_module(value):
    ...
    refs=[r for r in value['external_sources'] if Path(r['path']).name=='full_es_training.py']
    require(len(refs)==1,'One exact ES-only loader source required');reference=refs[0];verify_artifact(reference)
    name='_support_full_es_'+reference['sha256'][:12]
    if name not in sys.modules:
...
def validate_support_reconstruction(value,job,budget,partition):
